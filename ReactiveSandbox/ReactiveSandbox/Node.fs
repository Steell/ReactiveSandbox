...
let initialize_rect_drag (node : NodeUI) move_update =
    ...
    |> Observable.scanAccumulate world0 // take the result of the fired event and update the world
...
let new_node (window : MainWindow) move_update (position : Point) : UIElement =
    let node = NodeUI()
